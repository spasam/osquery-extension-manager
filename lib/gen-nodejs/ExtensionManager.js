...
const ExtensionManager_streamEvents_args = class {
  constructor(args) {
    ...
    if (args) {
      ...
      if (args.batch !== undefined && args.batch !== null) {
        this.batch = Thrift.copyList(args.batch, [Thrift.copyMap, null]);
      }
    }
  }
  ...
  write (output) {
    ...
    if (this.batch !== null && this.batch !== undefined) {
      output.writeFieldBegin('batch', Thrift.Type.LIST, 2);
      output.writeListBegin(Thrift.Type.MAP, this.batch.length);
      for (let iter68 in this.batch) {
        if (this.batch.hasOwnProperty(iter68)) {
          iter68 = this.batch[iter68];
...
const ExtensionManagerClient = exports.Client = class extends ExtensionClient {
  ...
  streamEvents (name, batch) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.send_streamEvents(name, batch);
    });
  }
...
const ExtensionManagerProcessor = exports.Processor = class extends ExtensionProcessor {
  ...
  process_streamEvents (seqid, input, output) {
    ...
    if (this._handler.streamEvents.length === 2) {
      ...
      this._handler.streamEvents(args.name, args.batch, (err, result) => {
        let result_obj;
        if ((err === null || typeof err === 'undefined')) {
