  ...
  async call(request) {
    if (!request.hasOwnProperty('action')) {
      return new types.ExtensionResponse({
        status: new types.ExtensionStatus({ code: 1, message: "Missing table request 'action'" })
      });
    } else if (request.action === 'generate') {
      let status = new types.ExtensionStatus({ code: 0 });
      try {
        const response = await this.generate();
        return new types.ExtensionResponse({ status, response });
      } catch (err) {
        return new types.ExtensionResponse({
    ...
    return new types.ExtensionResponse({
      status: new types.ExtensionStatus({ code: 1, message: "Unknown table request 'action': " + request.action })
    });
  }
